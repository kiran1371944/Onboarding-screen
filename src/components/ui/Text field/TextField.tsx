...
const inputVariants = cva(
  ' text-sm border transition-colors shadow-sm focus:outline-none focus:ring focus:ring-gray-200 disabled:opacity-50 disabled:pointer-events-none ',
  {
    variants: {
      variant: {
        default:
          'bg-slate-50 rounded-md border-gray-300 dark:border-slate-100 dark:bg-white text-white dark:text-slate-900 dark:hover:text-slate-100',
        danger:
          'bg-slate-50 rounded-md border-red-500 dark:border-red-500 dark:bg-white text-white dark:text-slate-900 dark:hover:text-slate-100',
        success:
          'bg-slate-50 rounded-md border-green-500 dark:border-green-500 dark:bg-white text-white dark:text-slate-900 dark:hover:text-slate-100',
      },
      size: {
  ...
  (
    ...
    return (
      <div>
        ...
        {sublabel ? (
          <label
            className={`block mb-2 mt-1 text-xs font-normal ${sublabelClassName}`}
          >
            {sublabel}
          </label>
        ) : (
